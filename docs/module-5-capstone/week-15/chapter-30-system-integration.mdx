...
graph TB
    subgraph "Perception Layer"
        A[camera_driver<br/>Multi-camera publisher]
        B[object_detector<br/>YOLOv8 TensorRT]
        C[depth_estimator<br/>DPT or RealSense]
        D[perception_fusion<br/>3D pose estimation]
    end

    subgraph "Planning Layer"
        E[vla_policy<br/>OpenVLA TensorRT]
        F[motion_planner<br/>MoveIt 2]
        G[navigation_stack<br/>Nav2]
        H[task_manager<br/>State machine]
    end

    subgraph "Control Layer"
        I[whole_body_controller<br/>Balance + coordination]
        J[arm_controller_left<br/>7-DOF trajectory]
        K[arm_controller_right<br/>7-DOF trajectory]
        L[locomotion_controller<br/>Stance/walk]
    end

    subgraph "Hardware Interface"
        M[robot_state_publisher<br/>TF tree]
        N[joint_state_publisher<br/>Sensor data]
        O[hardware_interface<br/>Motor drivers]
    end

    subgraph "Monitoring"
        P[safety_monitor<br/>Fall detection]
        Q[diagnostics<br/>Health checks]
        R[rviz2<br/>Visualization]
    end
...
**Progress**: Module 5, Week 15, Chapter 30 of 32 | [View all chapters](/intro)
